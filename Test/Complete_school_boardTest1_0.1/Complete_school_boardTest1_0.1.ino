...
int Index_WordToSay = -1; //Index of the word to say
const String WordToSay[NB_WORDS] = {"space","lxunch","stdent","xxx","???","xxx","xxx"};
...
unsigned long lastTimeListen = 0; //Date at which the Arduino started to listen
const int T_delayListen = 1 * 14 * 1000; // max time when the Arduino is in Listen mode
...
{   
    ...
    // Check if we just begin to be in receive mode
    if (ordreEcouteReceived == false){
        lastTimeListen = currentTime;
    }
    ...
    if (ordreEcouteReceived){
        ...
        if ((EndMessage(receivedMessageMorse, 7) == "/...-.-")) {
            ...
            addLog(String("Received message : ") + receivedMessagePlain);//DEBUG
            receivedMessageMorse = "";
            ...
            if (Translation == "error"){
                addLog(String("Error no translation for : ") + receivedMessagePlain);
                Index_WordToSay = (Index_WordToSay  + 1) % NB_WORDS;
                Translation = WordToSay[Index_WordToSay];
            }
            Index_WordToSay = (Index_WordToSay  + 1) % NB_WORDS;
            Translation = WordToSay[Index_WordToSay];
            MessageToSend = Translation;
            addLog(String("Next message to send : ") + Translation);
            Translation = "";
        ...
        if (((currentTime - lastTime0) > (dotDuration * 20)) && (receivedMessageMorse != "")) {
            ...
            Index_WordToSay = (Index_WordToSay  + 1) % NB_WORDS;
            MessageToSend = WordToSay[Index_WordToSay];
            addLog(String("Next message to send : ") + MessageToSend);
            ordreEcouteReceived = false; 
        }

        // If we are in Listen Mode for too long and we didn't receive anything
        if(((currentTime - lastTimeListen) > T_delayListen) && (ordreEcouteReceived == true)) {
            addLog(String("Error no message received") );
            Index_WordToSay = (Index_WordToSay  + 1) % NB_WORDS;
            MessageToSend = WordToSay[Index_WordToSay];
            addLog(String("Next message to send : ") + MessageToSend);
            ordreEcouteReceived = false; 
...
void initLDR()
{
   ...
   // Check ambient light for a certain period
    ...
    threshold = lightValueMean + 50;
    
    // Send Logs

    addLog("LDR Calibration Done.");
    addLog("Max Luminosity = " + String(lightValueMax));
    addLog("Mean Luminosity = " + String(lightValueMean));
    addLog("Threshold Value = " + String(threshold));
}
...
// Function that return the next wort to say based on the traduction of the word given in input
